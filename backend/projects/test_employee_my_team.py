...
class TestEmployeeMyTeamView:
    ...
    def test_bulk_query_does_not_scale_with_team_size(self):
        ...
        assert response.status_code == 200
        # 2 query cố định cho job+teammate data (jobs, task-rows) + các query
        # phụ của auth/permission middleware — chặn trần cao để không quá
        # giòn (fragile), nhưng vẫn đủ để bắt hồi quy N+1 rõ ràng (vd. nếu ai
        # đó lỡ query teammate riêng cho từng job trong vòng lặp).
