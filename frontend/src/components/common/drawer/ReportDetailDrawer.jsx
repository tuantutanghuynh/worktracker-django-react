...
export default function ReportDetailDrawer({
  ...
  const footerActions = (
    <div className="flex flex-wrap items-center justify-between w-full gap-3">
      <span className="text-xs text-slate-400">
        Secured export with automatic audit log entry
      </span>
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          disabled={isLoading}
          className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-lg bg-emerald-600/90 hover:bg-emerald-600 text-white shadow-md shadow-emerald-900/30 transition cursor-pointer"
        >
        ...
        <button
          ...
          disabled={isLoading}
          className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-lg bg-rose-600/90 hover:bg-rose-600 text-white shadow-md shadow-rose-900/30 transition cursor-pointer"
        >
  ...
  return (
    <SideDrawer
      ...
      subtitle={`Category: ${type} • Period: ${dateRange}`}
      size="xl"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Generated by <strong className="text-slate-200">{generatedBy}</strong> at {generatedAt || 'Today'}
            </p>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            Records Preview ({previewData.length} sample items)
          </h4>
          <span className="text-xs text-slate-500 italic">
            Showing up to 5 sample preview records
          </span>
        </div>

        {previewData.length === 0 ? (
          <div className="p-8 text-center border border-slate-800/80 rounded-xl bg-slate-950/40 text-xs text-slate-500">
            No sample records available for this report preview.
          </div>
        ) : (
