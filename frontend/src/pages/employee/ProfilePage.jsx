...
import { User, Mail, Phone, Building2, Camera, Save, RotateCcw, KeyRound } from "lucide-react";
...
export function ProfilePage() {
  ...
  const currentEmail = user?.email || profileData?.email || "user@worktracker.vn";

  const currentRole = (user?.role || profileData?.role?.code || profileData?.role || "EMPLOYEE").toUpperCase();
  ...
  const currentDepartment =
    ...
    (profileData?.department
      ? typeof profileData.department === "number"
        ...
        : profileData.department
      : user?.department || (currentRole === "ADMIN" ? "System & IT Operations" : "Staff Department"));

  const displayName = profileForm.fullName || user?.full_name || (currentRole === "ADMIN" ? "System Administrator" : "Staff Member");
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch'>
        ...
        <div className='lg:col-span-7 bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-8 shadow-xs flex flex-col justify-between'>
          <div>
            ...
            <form id='userProfileForm' onSubmit={handleSaveProfile} className='space-y-6'>
          ...
          <div className='flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 mt-6 border-t border-slate-100'>
            <span className='text-[11px] text-slate-400 italic text-center sm:text-left'>Changes will be synchronized immediately across all portals and dashboards.</span>
            <button
              type='submit'
